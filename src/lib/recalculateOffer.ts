...
export function recalculateFromSubmission(
  ...
  return { low, high, baseValue: Math.round(baseValue), totalDeductions: Math.round(deductions), reconCost: Math.round(reconCost), matchedRuleIds, isHotLead, marketAdjustment: 0, marketDaysSupply: null, strategyMode: "standard" as const, isCapped: false, projectedGross: 0, aiConditionAdjustment: aiAdj };
}
